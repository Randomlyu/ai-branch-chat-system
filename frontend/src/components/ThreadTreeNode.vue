  ...
  <div class="thread-node" :style="{ marginLeft: `${(thread.depth || 0) * 12}px` }">
    ...
    >
      ...
      <div class="thread-content">
        ...
        <!-- 标题区域 -->
        <div 
          class="thread-title-container" 
          :title="thread.title"
          :data-full-title="thread.title || getThreadTitle(thread)"
        >
          <input
            v-if="isRenaming"
            ref="renameInputRef"
...
.thread-title-container {
  flex: 1 1 auto;
  min-width: 0;
  overflow: visible; /* 改为 visible 确保工具提示可见 */
  font-size: 13px;
  color: #374151;
  padding: 2px 0;
  position: relative;
...
/* 悬停时显示完整标题的工具提示 */
.thread-title-container:hover::after {
...
/* 工具提示箭头 */
.thread-title-container:hover::before {
  content: '';
  position: absolute;
  left: 12px;
  top: 100%;
